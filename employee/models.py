...
class PartialTeamEmployeeRelation(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if PartialTeamEmployeeRelation.objects.filter( # that a leader already exists
            team=Team.objects.filter(name=self.team.name).first(),
            employee_type=self.Type.LEADER
        ).exists() and self.employee_type == self.Type.LEADER: # and that the current input tries to make a second leader
        ...
        if qs.exists():
            ...
            if total_hours + self.work_arr > total_work_arr:
                raise ValidationError(f'Employee {self.employee} cannot work more than 48 hours/week, exceeding by {total_hours + self.work_arr - total_work_arr}')
