...
class EmployeeApiTest(APITestCase):
    ...
    def setUpTestData(cls):
        cls.employee1 = Employee.objects.create(name="Employee1", hourly_rate=12, employee_id='A123')
        cls.employee2 = Employee.objects.create(name="Employee2", hourly_rate=13, employee_id='B123')
    ...
    def test_get_api(self):
        ...
        response_with_query = self.client.get(self.url, {'employee_id': 'A123'}, format='json')
        ...
        self.assertEqual(response_with_query.data, {'name': 'Employee1', 'hourly_rate': 12, 'employee_id': 'A123'})
    ...
    def test_post_api(self):
        ...
        print(f'Employee post data {response.data}')
        self.assertEqual(Employee.objects.all().count(), 3)
        self.assertEqual(Employee.objects.filter(name=employee['name']).first().name, 'Employee')
        self.assertEqual(Employee.objects.filter(hourly_rate=employee['hourly_rate']).first().hourly_rate, 14)
        self.assertEqual(Employee.objects.filter(employee_id=employee['employee_id']).first().employee_id, 'C123')

    def test_put_api(self):
        response_name = self.client.put('/api/employee/A123', {'name': 'Employee4'}, format='json') 
        self.assertEqual(response_name.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(Employee.objects.all().count(), 2)   
        self.assertEqual(Employee.objects.filter(id=self.employee1.id).first().name, 'Employee4')
        
        response_hourly_rate = self.client.put('/api/employee/A123', {'hourly_rate': 15.0}, format='json')  
        self.assertEqual(response_hourly_rate.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(Employee.objects.all().count(), 2)
        self.assertEqual(Employee.objects.filter(id=self.employee1.id).first().hourly_rate, 15.0)

        response_employee_id = self.client.put('/api/employee/A123', {'employee_id': 'D123'}, format='json')      
        self.assertEqual(response_employee_id.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(Employee.objects.all().count(), 2)
        self.assertEqual(Employee.objects.filter(id=self.employee1.id).first().employee_id, 'D123')
    ...
    def test_delete_api(self):
        ...
        self.assertEqual(Employee.objects.filter(id=self.employee1.id).first().name, 'Employee1')
        self.assertEqual(Employee.objects.filter(id=self.employee1.id).first().hourly_rate, 12)
        self.assertEqual(Employee.objects.filter(id=self.employee1.id).first().employee_id, 'A123')
...
class PartialTeamEmployeeApiTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.e1t2 = PartialTeamEmployeeRelation.objects.create(
            employee=cls.employee1, team=cls.team2, employee_type='EMPLOYEE', work_arr=16
            )
        cls.e2t1 = PartialTeamEmployeeRelation.objects.create(
            employee=cls.employee2, team=cls.team1, employee_type='EMPLOYEE', work_arr=18
            )
    ...
    def test_get_api(self):
        response = self.client.get(self.url, format='json')
        response_name = self.client.get(self.url, {'employee_id': 'A123'}, format='json')

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 4)

        self.assertEqual(response.data, [
            {'employee_type': 'LEADER', 'work_arr': 24, 'employee': 'A123', 'team': 'Team1'},
            {'employee_type': 'EMPLOYEE', 'work_arr': 16, 'employee': 'A123', 'team': 'Team2'},
            {'employee_type': 'EMPLOYEE', 'work_arr': 18, 'employee': 'B123', 'team': 'Team1'},
            {'employee_type': 'LEADER', 'work_arr': 30, 'employee': 'B123', 'team': 'Team2'}
        ])
        self.assertEqual(response_name.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response_name.data), 2)
        self.assertEqual(response_name.data, [
            {'employee_type': 'LEADER', 'work_arr': 24, 'employee': 'A123', 'team': 'Team1'},
            {'employee_type': 'EMPLOYEE', 'work_arr': 16, 'employee': 'A123', 'team': 'Team2'}
        ])

    def test_post_api(self):
        employee = Employee.objects.create(name="Employee", hourly_rate=14, employee_id='C123')
        ...
        # Team.save()
        e3t3 = {'employee_type': 'LEADER', 'work_arr': 20.0, 'employee': 'C123', 'team': 'Team'}
        e3t1 = {'employee_type': 'EMPLOYEE', 'work_arr': 20.0, 'employee': 'C123', 'team': 'Team1'}
        ...
        self.assertEqual(PartialTeamEmployeeRelation.objects.filter(
            employee=employee.id, team=self.team1.id).first().employee.employee_id, e3t1['employee']
            )
        self.assertEqual(PartialTeamEmployeeRelation.objects.filter(
            employee=employee.id, team=self.team1.id).first().employee_type, e3t1['employee_type']
            )
        self.assertEqual(PartialTeamEmployeeRelation.objects.filter(
            employee=employee.id, team=self.team1.id).first().team.name, e3t1['team']
            )
        self.assertEqual(PartialTeamEmployeeRelation.objects.filter(
            employee=employee.id, team=self.team1.id).first().work_arr, e3t1['work_arr']
            )
        ...
        self.assertEqual(PartialTeamEmployeeRelation.objects.filter(
            employee=employee.id, team=team.id).first().employee.employee_id, e3t3['employee']
            )
        self.assertEqual(PartialTeamEmployeeRelation.objects.filter(
            employee=employee.id, team=team.id).first().employee_type, e3t3['employee_type']
            )
        self.assertEqual(PartialTeamEmployeeRelation.objects.filter(
            employee=employee.id, team=team.id).first().team.name, e3t3['team']
            )
        self.assertEqual(PartialTeamEmployeeRelation.objects.filter(
            employee=employee.id, team=team.id).first().work_arr, e3t3['work_arr']
            )
        

    def test_put_api(self):
        response_employee = self.client.put(self.url, {'employee_pk': 'A123', 'team_pk': 'Team1', 'employee_type': 'EMPLOYEE'}, format='json')
        ...
        ).first().employee_type, 'EMPLOYEE')

        response_arr = self.client.put(self.url, {'employee_pk': 'A123', 'team_pk': 'Team1', 'work_arr': 5}, format='json')
        self.assertEqual(response_employee.status_code, status.HTTP_204_NO_CONTENT)
        self.assertEqual(PartialTeamEmployeeRelation.objects.all().count(), 4)
        self.assertEqual(PartialTeamEmployeeRelation.objects.filter(
            employee=self.employee1.id, team=self.team1.id
    ...
    def test_delete_api(self):
        response = self.client.delete(self.url, {'employee_pk': 'B123', 'team_pk': 'Team2'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
...
class FinancialsApiTest(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.e2t1 = PartialTeamEmployeeRelation.objects.create(
            employee=cls.employee2, team=cls.team1, employee_type='EMPLOYEE', work_arr=16
            )
        cls.e2t2 = PartialTeamEmployeeRelation.objects.create(
    ...
    def test_get_api(self):
        ...
        response = self.client.get(self.url, format='json')
        response_employee = self.client.get(self.url, {'employee_id': 'A123'}, format='json')
        response_team = self.client.get(self.url, {'team': 'Team1'}, format='json')
        response_employee_team = self.client.get(self.url, {'employee_id': 'A123', 'team': 'Team1'}, format='json')
        ...
        self.assertEqual(response_employee.data, 
